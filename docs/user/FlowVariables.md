...
| <a name="GLOBAL_ROUTE_ALLOW_CONGESTION"></a>GLOBAL_ROUTE_ALLOW_CONGESTION| Passed as -allow_congestion to EVERY global_route call in the stage (the initial route and each -end_incremental re-route). Without it, a design that global routing cannot make congestion-free ENDS THE STAGE with an error (GRT-0116 from the batch route, GRT-0232 from the incremental one), so the run produces no global-route metrics at all -- the congestion is a gate, not a measurement. With it set to 1 both become warnings, the stage finishes, and the congestion that could not be resolved is reported (congestion report files plus the globalroute overflow metrics). Intended for studies that need to MEASURE congestion instead of gating on it: the guides handed to detailed routing really are congested, and detailed routing may fail on them.| 0|
...
- [GLOBAL_ROUTE_ALLOW_CONGESTION](#GLOBAL_ROUTE_ALLOW_CONGESTION)
